...
const normalizeTagForFileName = (tag) =>
  tag.replaceAll(/[^a-zA-Z0-9._-]/g, "-")
...
const createReleaseAssets = async ({
  ...
  const archiveBinaryDirectoryPath = path.join(
    archiveStagingDirectoryPath,
    "bin"
  )
...
const createJsonResponseWriter = (response) => (statusCode, payload) => {
...
const createMockReleaseServer = async ({
  ...
  for (const record of releaseAssetRecords) {
    downloadPayloadByFileName.set(
      record.archiveName,
      await readFile(record.archivePath)
    )
