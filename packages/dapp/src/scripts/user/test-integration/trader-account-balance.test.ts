...
describe("trader-account-balance script", () => {
  it("renders funded balance-manager assets with withdraw-ready arguments", async () => {
    await testEnv.withTestContext("user-trader-account-balance", async (context) => {
      ...
      const fundResult = await scriptRunner.runUserScript("trader-account-fund", {
        account: trader,
        args: {
          ammPackageId,
      ...
      if (!suiAsset) {
        throw new Error("Expected trader-account-balance output to include SUI.")
      }

      expect(suiAsset.balance).toBe(fundingAmount.toString())
      expect(suiAsset.withdrawArguments.coinType).toBe(NORMALIZED_SUI_COIN_TYPE)
      expect(suiAsset.withdrawArguments.amount).toBe(fundingAmount.toString())
      expect(suiAsset.withdrawCommand).toContain(
        "pnpm dapp user:trader-account:withdraw"
      )
      expect(suiAsset.withdrawCommand).toContain(
        `--trader-account-id ${traderAccount.traderAccountId}`
      )
      expect(suiAsset.withdrawCommand).toContain(
        `--coin-type ${NORMALIZED_SUI_COIN_TYPE}`
      )
      expect(suiAsset.withdrawCommand).toContain(
        `--amount ${fundingAmount.toString()}`
      )
