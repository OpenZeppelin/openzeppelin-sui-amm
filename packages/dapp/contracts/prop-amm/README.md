...
- `AdminCap` (owned object): capability required to update market/config and manage balances for a specific market maker executor.
...
7. To replace the traded pool or Pyth feeds, pause the executor, then pass a new `Market` to `executor::update_market`.
