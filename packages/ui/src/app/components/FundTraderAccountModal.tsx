...
import {
  ...
  type TraderAccountFundSummary,
  type WalletCoinBalanceOption
} from "../hooks/useFundTraderAccountModalState"
...
const resolveCoinOptionLabel = (walletCoinBalance: WalletCoinBalanceOption) =>
  `${getStructLabel(walletCoinBalance.coinType)} (${walletCoinBalance.totalBalance.toString()} base units)`

const FundSummarySection = ({
  summary,
  explorerUrl
}: {
  summary: TraderAccountFundSummary
  explorerUrl?: string
}) => (
  <ModalSection
...
const FundSuccessView = ({
  ...
  <>
    <ModalStatusHeader
      status="success"
      title="Trader account funded"
      subtitle={shortenId(summary.traderAccountId)}
      description="The funding transaction has been confirmed on-chain."
      onClose={onClose}
    />
    <ModalBody>
      <FundSummarySection summary={summary} explorerUrl={explorerUrl} />
      <TransactionRecap
      ...
      <ModalBody>
        {traderAccountId && balanceManagerId ? (
          <ModalSection
            ...
            subtitle="Funds are deposited into this trader account balance manager."
          >
        ...
        >
          <label className={modalFieldLabelClassName}>
            ...
            {walletCoinBalancesState.status === "loading" ? (
              ...
              >
                {walletCoinBalancesState.status === "success"
                  ? walletCoinBalancesState.balances.map(
                      (walletCoinBalance) => (
                        <option
                          key={walletCoinBalance.coinType}
                          value={walletCoinBalance.coinType}
                        >
                          {resolveCoinOptionLabel(walletCoinBalance)}
                        </option>
                      )
                    )
                  : null}
          ...
          {selectedWalletCoinBalance ? (
            <div className="rounded-xl border border-slate-200/70 bg-white/80 px-3 py-3 text-xs dark:border-slate-50/15 dark:bg-slate-950/60">
              ...
              <div className="mt-1 font-semibold text-sds-dark dark:text-sds-light">
                {selectedWalletCoinBalance.totalBalance.toString()} base units
              </div>
              <div className="mt-1 text-[0.65rem] text-slate-500 dark:text-slate-200/70">
                {selectedWalletCoinBalance.coinObjectCount} coin object
                {selectedWalletCoinBalance.coinObjectCount === 1 ? "" : "s"}
              </div>
            </div>
          ) : null}
        </ModalSection>
...
export default FundTraderAccountModal
