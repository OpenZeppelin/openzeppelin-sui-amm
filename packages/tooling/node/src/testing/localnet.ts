...
const collectLocalDependencyEntries = (
  ...
  const localDependencyEntry =
    /^\s*([A-Za-z0-9_-]+)\s*=\s*\{[^}]*\blocal\s*=\s*"([^"]+)"[^}]*\}.*$/

  lines.forEach((line) => {
    const entryMatch = line.match(localDependencyEntry)
...
const collectLocalDependencyEntriesInDependencySections = (
  ...
  lines.forEach((line) => {
    ...
    if (headerMatch) {
      ...
      if (headerName === "dependencies" || headerName === "dev-dependencies") {
...
const stripLocalnetDepReplacementsForLocalDeps = async (
  ...
  const localDeps = collectLocalDependencyNames(contents)
  if (localDeps.size === 0) return

  const lineEnding = resolveTomlLineEnding(contents)
  const lines = contents.split(/\r?\n/)
  const sectionHeaderRegex = /^\s*\[dep-replacements\.localnet\]\s*(#.*)?$/
  ...
  const entryRegex = (dependencyName: string) =>
    new RegExp(
  ...
  const filteredSectionLines = lines
    ...
    .filter((line) => {
      for (const dependencyName of localDeps) {
        if (entryRegex(dependencyName).test(line)) {
          didUpdate = true
          return false
        }
      }
  ...
  await writeFile(moveTomlPath, updatedLines.join(lineEnding), "utf8")
}
  ...
  if (isPathWithinRoot(destinationMoveTomlPath, externalRoot)) {
    for (const [
      ...
      if (isPathWithinRoot(destinationMoveTomlPath, destinationRoot)) {
        const relative = path.relative(destinationRoot, destinationMoveTomlPath)
...
const rewriteMoveTomlLocalDependencyEntries = ({
  ...
  const updatedLines = lines.map((line) => {
    ...
    const replacementPath = dependencyReplacements.get(dependencyName)
